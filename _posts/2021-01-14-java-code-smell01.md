...
1. 객체를 매개변수로 전달 가능해야 한다. 
2. 리턴 값으로 반환 할 수 있어야 한다.
3. 메소드의 인자로 전달 가능해야 한다. 

간략하게 정리하자면 우리가 일반적으로 만들고 사용해온 객체는 모두 일급 객체라고 할 수 있습니다. 

일급 함수란 무엇인가 ? 
`함수형 프로그래밍의 관점에서는 함수를 마치 일반값처럼 사용해서 인수로 전달하거나, 결과로 반환받거나, 자료구조에 저장할 수 있음을 의미하고 일반값처럼 취급할 수 있는 함수를 일급함수라고 한다.` 
